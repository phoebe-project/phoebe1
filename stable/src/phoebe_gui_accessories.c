...
void parse_startup_line (int argc, char *argv[])
	...
	for (i = 1; i < argc; i++)
		...
		if ( (strcmp (argv[i],  "-h"   ) == 0) ||
			...
			{
			printf ("\nPHOEBE 0.28 command line arguments: [-hsv] [keyword_file]\n\n");
		...
		if ( (strcmp (argv[i],  "-v"      ) == 0) ||
			...
			{
			printf ("\nPHOEBE 0.28 (stable release), November 1st, 2005 by Andrej Prsa\n");
...
void phoebe_init ()
	...
	sprintf (PHOEBE_VERSION_NUMBER, "0.28 (stable release)");
	sprintf (PHOEBE_VERSION_DATE,   "November 1st, 2005");
  ...
  PHOEBE_configuration_browse = create_PHOEBE_configuration_browse ();
	...
	sprintf (dest_str,   "%s/atmcofplanck.dat", PHOEBE_TEMP_DIR);
	sprintf (source_str, "%s/atmcofplanck.dat", PHOEBE_LC_DIR);
	if (file_exists (source_str))
		{
		if (!file_exists (dest_str))
			symlink (source_str, dest_str);
