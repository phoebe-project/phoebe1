...
double phoebe_chi2_cost_function (PHOEBE_vector *adjpars, PHOEBE_nms_parameters *params)
{
	...
	for (i = 0; i < lcno + rvno; i++) {
		...
		phoebe_compute_lc_using_wd (curve, obs[i]->indep, lcin, args[i], NULL, NULL, NULL, NULL, NULL, NULL);
		remove (lcin);
		free (lcin);
...
int phoebe_minimize_using_dc (FILE *dc_output, PHOEBE_minimizer_feedback *feedback)
{
	...
	for (i = 0; i < rvno; i++)
		feedback->chi2s->val[i] = 100.*100.*chi2s[i]/params->sigma[i]/params->sigma[i];
