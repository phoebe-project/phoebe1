...
void parse_startup_line (int argc, char *argv[])
{
	...
	for (i = 1; i < argc; i++) {
		if ( (strcmp (argv[i],  "-h"   ) == 0) ||
			...
			printf ("\nPHOEBE 0.29d command line arguments: [-hsv] [keyword_file]\n\n");
		...
		if ( (strcmp (argv[i],  "-v"      ) == 0) ||
			...
			printf ("\nPHOEBE 0.29d (stable release), September 25, 2007\n");
...
void phoebe_init ()
	...
	sprintf (PHOEBE_VERSION_NUMBER, "0.29d (stable release)");
	sprintf (PHOEBE_VERSION_DATE,   "September 25, 2007");
